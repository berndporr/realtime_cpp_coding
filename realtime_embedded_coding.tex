...
Here, the publishing class directly has a fuctor object of type \textsl{std::function}
from the standard template class.
A functor typically stores lambda functions which
are here the glue between the publisher and subscriber(s). We use as an example
an ADC which wants to publish its voltage readings via a callback, implemented with
a functor.
...
Another popular approach is creating an additional class as a glue between
publisher and subscriber which is called \textsl{interface
  class}: publisher $\to$ interface $\to$ subscriber.

This class has typically only one \textsl{abstract} method which is
the event callback and then needs to be implemented by the subscriber or containing
a way of calling then the specific subscriber.
...
which is then implemented by the subscriber.
As an example the subscriber could just inherit the interface class and print \texttt{sample}:
\begin{verbatim}
...
\end{verbatim}
where here the abstract method is overloaded by a simple printer. This
decouples the publisher and subscriber.

The publisher on the other hand
receives a pointer to the instance of the interface:
\begin{verbatim}
...
...
Thus, the glue between publisher and subscriber is established by
creating an additional interface class which contains
an abstract method which then needs to be implemented being useful to the
subscriber class or is the subscriber itself as in this example.
This is a very efficient way and the pure C++ way. However,
it requires pointers which might pose a safety risk.
