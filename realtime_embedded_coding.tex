...
in realtime over a network. The Data Distribution Service (DDS)
is a middleware protocol and API which has been developed to provide reliable and low-latency publisher-subscriber communication (see Fig.~\ref{dds1}). The publisher sends a data structure
(here a C++ \texttt{struct}) and then at the receiver a callback is triggered
delivering the data structure.
We focus on fastDDS which uses the RTPS (Real Time Publish Subscribe) protocol.
It offers plug and play connectivity which won't require to specify IP addresses in a local networks so that applications are automatically discovered. fastDDS is now the standard middleware of ROS2, the popular robotic operating system.
...
\section{Publisher}
Write a publisher class which publishes the message \texttt{HelloWorldMsg}:
...
\end{verbatim}
whenever \texttt{publish} is called.
...
\end{verbatim}
which has the callback \texttt{on\_data\_available}
which is called after the publisher has sent a message.
