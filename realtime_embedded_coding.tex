...
This handout is about Realtime Embedded Coding under Linux
in its userspace. A lot of applications with moderate sampling
rates such as robots but even sound or video applications
won't need kernel based code. In these instances the userspace
is the best choice as it provides application safety and at the
same time still offers realtime event handling.
...
a sensor signalling that a sample is ready to be picked up.
...
Events between C++ classes are transmitted via callbacks. This is basically a
publisher / subscriber framework.
There are different ways of tackling the issue of callbacks and we are presenting
two approaches: functor and interface classes.

\subsection{\texttt{std::function} (functors)}
Here, the publishing class directly has a functor object of type \textsl{std::function}
from the standard template class.
A functor typically stores lambda functions. Here, these
are the glue between the publisher and subscriber(s). We use as an example
an ADC which publishes its voltage readings via a callback, implemented with
a functor.
See \url{https://github.com/berndporr/rpi_ads1115} for the complete example.
\paragraph{Publishing class}
In the publisher class we define its type as:
\begin{verbatim}
using ADSCallbackInterface = std::function<void(float)>;
\end{verbatim}
which defines a functor which receives the voltage reading of the ADC as a float.
Within the ADC class we create an instance variable:
\begin{verbatim}
class ADS1115rpi {
  ...
...
\end{verbatim}
which contains now the functor. This is called every time an ADC measurement has been taken.
We also added the method \texttt{registerCallback} where we register a lambda function
which the transmits the data to the subscriber.

\paragraph{Subscriber}
The class subscribing to the ADC simply needs to have a method which receives
a floating point:
\begin{verbatim}
...
\end{verbatim}
which is here: \texttt{hasADS1115Sample} and just prints the ADC values as an example.

\paragraph{Connecting publisher and subscriber: lambda function}
In the main program we connect the publisher and subscriber with a
lambda function:
\begin{verbatim}
...
\end{verbatim}
The capture \texttt{[\&]} in the lambda function makes sure that \texttt{ads1115Printer}
is available to the lambda function which then allows calling \texttt{hasADS1115Sample}
of the instance \texttt{ads1115Printer} from the instance \texttt{ads1115rpi} and, thus,
establishing elegantly the glue between these two classes. Adding more subscribers
is now also straightforward.


\subsection{Interface classes with abstract functions}
Another popular approach is creating an \textsl{additional} class as the glue between
publisher and subscriber which is called \textsl{interface
  class}: publisher $\to$ interface $\to$ subscriber.

This class has typically only one virtual \textsl{abstract} method which is
the event callback and needs to be implemented by the subscriber.
...
\end{verbatim}
which contains the abstract method \texttt{hasADS1115Sample(float sample) = 0;}
which must be implemented by the subscriber.
In the simplest case the subscriber could just inherit the interface
class and overloads \texttt{hasADS1115Sample},
for example:
\begin{verbatim}
...
\end{verbatim}
where here the abstract method is overloaded by a simple printer. One could
also keep the interface class separate from the subscriber class but adding
a reference to the subscriber which is then called from \texttt{hasADS1115Sample}.
...
...
\end{verbatim}
where \texttt{adsCallbackInterface} is a pointer of type
\texttt{ADSCallbackInterface}.

The main program then creates an instance of the publisher and an instance of the subscriber
...
This is a very efficient way and the pure C++ way which can easily be optimised by the C++
compiler. However, it's more difficult to have more than one subscriber which, for example,
would require an array of subscribers in the publisher and is not very flexible. Lambda
functions as callbacks offer a lot more flexible way and can easily call multiple subscribers
...
The following sections we explain of how to write
the C++ classes using different hardware protocols which then emit events via C++ callbacks.

To emulate an ``interrupt'' or event in userspace we use
\textsl{blocking I/O} which provides the timing, for example, when
waiting for a GPIO change, doing audio or video I/O.

Generally any userspace program talks to the kernel via pseudo files
in the \texttt{/dev} or \texttt{/sys} directories.
...
can be used to detect events! These are basically falling or rising edges at a GPIO pin.
GPIO is useful to turn non-blocking I/O operations into blocking ones:
SPI and I2C are only blocking for their read/write operations but they don't know
when new data is available at a sensor. However, most sensors have a DATA-READY output
which is then used to create events which are detected via the GPIO.

Use the commandline tool
\texttt{gpioinfo} to print a list of all available GPIO pins.

The GPIO can easily be accessed via
the \texttt{/dev} filesystem and the \texttt{libgpiod} v2. Here we use its C++ API:
\begin{enumerate}
...
\end{verbatim}
and then,
\item select a pin with \texttt{pinNo} on the chip and tell it that you want to detect events on it:
\begin{verbatim}
...
\end{verbatim}
\item Build a request which deals with the events:
\begin{verbatim}  
...
\end{verbatim}
\item Start a thread containing a \texttt{worker} which waits for events:
\begin{verbatim}  
void worker() {
  running = true;
  while (running) {
...
\end{verbatim}
which then calls the callback \texttt{myGpioEventCallback} after a rising or falling
edge at the GPIO pin \texttt{pinNo}.
\end{enumerate}
See a complete example here: \url{https://github.com/berndporr/libgpiod_event_demo}.
...
reader_ = subscriber_->create_datareader(topic_,
                                         ...
                                         &listener_);
\end{verbatim}
where \texttt{listener} is a callback interface which is called whenever data is available:
\begin{verbatim}
