...
via its pseudo devices. Note that from bookworm onwards that is done via
gstreamer.
\begin{verbatim}
while(running) {
    cv::Mat cap;
...
Both reading and writing to GPIO is \textsl{not blocking} but we need to be able to block on GPIO input events.
This is done with the ``poll'' command:
\begin{verbatim}
