...
C++ \texttt{struct}) through a communcation channel which is called
``topic''. At the subscriber side this message is received via the topic and then triggers a callback
delivering that message.  We focus on fastDDS which uses the
...
\section{Publisher}
Create a topic which associates the struct of the message with a human readable
name:
\begin{verbatim}
...
and with that the datawriter:
\begin{verbatim}
writer_ = publisher_->create_datawriter(topic_,
                                        DATAWRITER_QOS_DEFAULT,
                                        &listener_);
\end{verbatim}
where the \texttt{listener\_} is a callback interface derived from \texttt{DataWriterListener}
which signals when a subscriber has been found matching the topic.

Package this all in a class which publishes the message \texttt{HelloWorldMsg}:
