...
For a full coding example ``arecord'' is a good start. Both can be found here:
...
\item Endless loops with blocking I/O or GPIO wakeups
  to establish \textbf{precise timing}.
...
\subsection{Constant sampling rate output (audio, etc)}
For example audio DACs require a constant sampling rate and thus
constant output. In this case the C++ driver class will have again
a blocking loop and then read a buffer populated by the setter,
which is ideally always updated by the setter. Of course you
can also implement a callback by the audio to \textsl{request} samples
but needs to be ultimately resolved in a class between
audio arriving and being dispatched.
