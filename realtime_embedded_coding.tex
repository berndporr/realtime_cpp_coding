...
\end{verbatim}
and then,
\item as a 2nd step select a pin on the chip:
\begin{verbatim}
...
command:
\begin{verbatim}
int ret = gpiod_line_request_rising_edge_events(pinSwitch, "Consumer1");
\end{verbatim}
...
kernel event to userspace and clears it out of the kernel event queue. It's important
to do the read operation even if the event-struct is never used. Then call
