...
back into callbacks between classes. Data back to the hardware
is transmitted via methods, called ``setters''.
...
\item \textsl{Dependency inversion}: That is about obstructing the
  ...
  you want, for example, a base class for covering a range of similar
...
\begin{enumerate}
\item the project has a a \textbf{build system} such as cmake. It's strongly recommended to use cmake
  (autoconf only for older existing projects).
\item classes (in particular driver classes) are \textbf{re-usable} outwith of the specific project and have
...
or /sys which means that a read operation blocks till data has arrived
