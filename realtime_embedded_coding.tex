...
\section{Conclusion}
Threads play a very important role in realtime coding as they
establish together with blocking I/O the callbacks so that every
event triggers separate threads and with that distributes the computing
load over many processors.

Callbacks are of course also used to signal the termination of
threads and shows again the close relationship between threads
and callbacks.
...
\section{Conclusion}
Events in QT are generated by user interaction, for
example a button press or a mouse. As before QT provides
a callback mechanism. Timers in QT are used for animations
but must not be used for realtime events as QT timers
won't guarantee a reliable timing but rather adjust to
CPU load.
...
\section{Conclusion}
Events in web based communication are always triggered by the
web browser in exactly the same way as QT does it. These events
either transmit data from the web browser to the web server or request
data from the web server. The protocol is nowadays always http or https
and is a RESTful interface with JSON the most popular data format
being requested.
...
device. Setters should, as callbacks, abstract away from the hardware.
