...
So far communicaton has only been between classes within the same
program on the same computer. However, often data needs to be transmitted
in realtime to other computers. The Data Distribution Service (DDS)
is a middleware protocol and API which has been developed to provide low-latency publisher-subscriber communication, extreme reliability, and a scalable architecture (see Fig.~\ref{dds1}). The publisher publishes a data structure
(here a C++ struct) and then at the receiver a callback will be triggered
delivering the data.
We focus on fastDDS which uses the RTPS (Real Time Publish Subscribe) protocol.
It offers plug and play connectivity which won't require to specify IP addresses in a local network so that any new application is automatically discovered. fastDDS is now the standard middleware of ROS2, the popular robotic operating system.

Here are the steps how to use fastDDS.

\section{Define the data structure}

The idea with fastDDS is to specify a data structure as a
template which is then converted into a header and cpp file.
...
\end{verbatim}
contains a number and a C++ string. This template file is then
converted to two C++ header and cpp files with the helper:
\begin{verbatim}
fastddsgen HelloWorldMsg.idl
\end{verbatim}
which of course should be part of your cmake build system.
Include the headers into your publisher and receiver.

\section{Publisher}

Write a publisher class which publishes the message:
...
Write a subscriber which listens to the published topic:
\begin{verbatim}
...
\end{verbatim}
which has a callback which is called after the publisher has sent a message.
