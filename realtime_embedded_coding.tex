...
\section{Conclusion}
In conclusion, the communication between C++ classes is achieved via
callbacks and setters where the event from the sensor traverses
through the C++ classes via callbacks and then back to the control
output via setters.
...
\section{Conclusion}
Threads play a central role in realtime coding as they establish
together with blocking I/O the callback interfaces so that every event
triggers a separate thread.
...
via the ``connect'' method. Callbacks from QT timers are used for
animations but must not be used for realtime events as QT timers won't
...
browser in exactly the same way as QT does it, for example by a button
press. These events either transmit data from the web browser to the
web server or request data from the web server. The protocol is
