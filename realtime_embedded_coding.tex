...
news is that \texttt{libcamera} already implements a callback
...
\begin{enumerate}
\item first choose the I/O \textsl{chip} either by its number, name or pseudo-file:
\begin{verbatim}
  struct gpiod_chip *chip = gpiod_chip_open_by_name("gpiochip0");
\end{verbatim}
and then,
\item as a 2nd step you select the pin on the chip:
\begin{verbatim}
  struct gpiod_line *pinRed = gpiod_chip_get_line(chip, 24);
\end{verbatim}
\end{enumerate}

\subsubsection{Output}
Once you have a pointer to the pin then you can set its direction, for example
...
For event progressing with need blocking I/O which then wakes up a thread.
This means that a change in level from low to high or high
to low unblocks an I/O operation, the underlying thread wakes up and
then a callback can be triggered.

To request event processing on a pin simply request it with this single
command:
\begin{verbatim}
int ret = gpiod_line_request_rising_edge_events(pinSwitch, "Consumer");
\end{verbatim}
This will turn the GPIO pin \texttt{pinSwitch} into an input pin and enables
blocking I/O on this pin.

Then start a tread where its worker contains:
\begin{verbatim}
void worker() {
	while (running) {
...
pin and then unblocks. Then \texttt{gpiod\_line\_event\_read} transfers the
kernel event to userspace and clears it out of the kernel space. It's important
to do the read operation even, if the struct is never used. Then call
the callback and repeat until the user terminates the thread.
